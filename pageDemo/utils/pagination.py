# _*_coding:utf-8_*_
'''
分页组件：这个是自己自定义的pagination组件
以后当我们要使用分页的时候，可以直接复制这个组件去调用
这个不依赖于任何组件和接口
'''


class Pagination(object):
    def __init__(self, current_page, all_count, base_url, query_aprams,
                 per_page=20, pager_page_count=11):
        '''
        分页初始化
        :param current_page: 当前页码
        :param all_count: 每页显示数据条数
        :param base_url: 数据库中总条数
        :param query_aprams: 基础URL
        :param per_page: QueryDict对象，内部含所有当前URL的原条件
        ...
        try:
            ...
            self.current_page = 1
        self.query_parmas = query_aprams
