...
from .serializers import CourseChapterSerializer, CourseCommentSerializer, QuestionSerializer
...
class CourseView(APIView):
    def get(self, request):
        # 获取过滤条件中的分类ID
        category_id = request.query_parmas.get("category", 0)
        # 根据分类获取课程
        if category_id == 0:
...
class CourseChepterView(APIView):
    def get(self, request, pk):
        # ['第一章': {课时一， 课时二}]
