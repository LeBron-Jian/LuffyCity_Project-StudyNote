...
class CourseDetailSerializer(serializers.ModelSerializer):
    ...
    def get_course_outline(self, obj):
        return [{'id': outline.id, 'title': outline.title, 'content': outline.content}
                for outline in obj.course_outline.all().order_by('order')]

    def get_price_policy(self, obj):
        return [{'id': price.id, 'valid_price_display': price.get_valid_period_display(),
                 'price': price.price} for price in obj.course.price_policy.all()]
    ...
    class Meta:
        ...
        fields = ['id', 'hours', 'summary', 'level', 'study_num', 'recommend_courses',
                  'teachers', 'price_policy', 'course_outline']
...
class CourseChapterSerializer(serializers.ModelSerializer):
    ...
    def get_sections(self, obj):
        return [{'id': section.id, 'title': section.title, 'free_trail': section.free_trail}
