...
class SearchGroupRow(object):
    ...
    def __iter__(self):
        ...
        for item in self.queryset_or_tuple:
            ...
            # 需要request.GTE  gender=1 &depaer=2
            # 获取组合搜索按钮文本背后对应的值
...
class Option(object):
    def __init__(self, field, is_multi=False, db_condition=None, text_func=None, value_func=None):
        ...
        if not db_condition:
            db_condition = {}
        self.db_condation = db_condition
        self.text_func = text_func
        self.value_func = value_func
        self.is_choice = False

    def get_db_condation(self, request, *args, **kwargs):
        return self.db_condation
    ...
    def get_queryset_or_tuple(self, model_class, request, *args, **kwargs):
        ...
        if isinstance(field_object, ForeignKey) or isinstance(field_object, ManyToManyField):
            # FK 和 M2M 应该去获取其关联表中的数据
            db_condition = self.get_db_condation(request, *args, **kwargs)
            return SearchGroupRow(title, field_object.remote_field.model.objects.filter(**self.db_condation), self,
                                  request.GET)
        else:
            # 获取choice中的数据, 元组
            self.is_choice = True
...
class StarkHandler(object):
    ...
    def changelist_view(self, request, *args, **kwargs):
        ...
        # print(self.model_class, data_list)
        body_list = []
    ...
    def reverse_add_url(self):
        ...
        if not self.request.GET:
            ...
            param = self.request.GET.urlencode()  # page=2&age=19
...
class StarkSite(object):
    ...
    def register(self, model_class, handler_class=None, prev=None):
        ...
        self._registry.append({'model_class': model_class,
